    ...
    content.innerHTML = `
        <div class="flex flex-col gap-6">
            ...
            <div class="w-full text-left">
                ...
                <p class="text-gray-400 text-[12px] md:text-xs leading-relaxed uppercase whitespace-pre-line mb-5 bg-white/5 p-3 border-l border-red-600">
...
window.updateProfile = async () => {
    ...
    window.primeShow("მონაცემები განახლებულია პრაიმ ბაზაში");
...
window.order = async (id, name) => {
    ...
    if(!data.phone || !data.address) { 
        window.primeShow("შეკვეთისთვის საჭიროა პროფილის სრულად შევსება!", false); 
        window.toggleProfile(); 
        return; 
    }
    ...
    // თანხმობის მოდელი
    window.primeShow(
        `ადასტურებთ შეკვეთას: ${name}? თანხმობის შემთხვევაში მოთხოვნა გადაეცემა ოპერატორს.`, 
        true, 
        async () => {
            ...
            window.primeShow("შეკვეთა მიღებულია. ოპერატორი დაგიკავშირდებათ.", false);
        }
    );
};
...
window.handleLogin = async () => {
    ...
    try { 
        ...
        window.primeShow("ავტორიზაციის შეცდომა: " + e.message); 
    }
};
...
window.handleRegister = async () => {
    ...
    try {
        ...
        window.primeShow("რეგისტრაციის შეცდომა: " + e.message); 
    }
};
